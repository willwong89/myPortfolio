...
//path fixing
for(int i=0 ; i<flagButtonIconPath.length ; i++){ flagButtonIconPath[i] = path+flagButtonIconPath[i]; }
for(int i=0 ; i<connectedIconPath.length ; i++){ connectedIconPath[i] = path+connectedIconPath[i]; }
for(int i=0 ; i<arrowIconPath.length ; i++){ arrowIconPath[i] = path+arrowIconPath[i]; }
randomizePlotColorButtonIconPath = path+randomizePlotColorButtonIconPath;
...
void setup(){
  ...
  data = loadStrings( path+"data/daily-foreign-exchange-1983-1985.csv" );
